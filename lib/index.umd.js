	...
	***************************************************************************** */
	...
	function registerMicroAppsConfig(microApps, option) {
	  ...
	  microApps.forEach(apps => {
	    const entry = env === 'dev' && devParam && devParam.find(({
	      key,
	      url
	    }) => key === apps.name) || null;
	...
	class UseMicroApp {
	  ...
	  render(appProps = {}) {
	    ...
	    if (_self.$log) {
	      ...
	      BrowserLogColor.bgBlue('初始化实例', table);
	    }
	  ...
	  start() {
	    ...
	    if (_self.$log) {
	      BrowserLogColor.bgGroup(['启动', `${_self.$name}:`], ['bgSpringGreen', 'bgBlack']);
	      const table = {
	      ...
	      BrowserLogColor.bgBlue('应用参数', table);
	    }
